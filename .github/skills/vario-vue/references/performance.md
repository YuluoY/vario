...
2. [path-memo（路径缓存）](#path-memo)
3. [子树组件化](#子树组件化)
4. [循环组件化](#循环组件化)
5. [SchemaStore（Schema 碎片化）](#schemastore)
...
源码：`packages/vario-vue/src/features/schema-store.ts`

将整棵 schema 树拆分为 `path → reactive(node)` 的扁平映射，修改某路径的节点只触发依赖该路径的 effect。SchemaStore 由渲染器内部管理。
...
- SchemaStore 为内部数据结构，通过 Schema Query API 间接使用
