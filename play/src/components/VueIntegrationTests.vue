      ...
      <el-col :xs="24" :md="12">
        <el-card class="test-card">
          ...
          <div class="render-output">
            ...
            <!-- Debug info -->
            <div v-if="renderVNode" style="margin-top: 10px; padding: 10px; background: #f5f5f5; font-size: 12px; border-radius: 4px;">
      ...
      <el-col :xs="24" :md="12">
        <el-card class="test-card">
          ...
          <div class="state-actions">
            ...
            <el-form :model="stateUpdateForm" label-width="100px">
        ...
        <el-card class="test-card">
          ...
          <div class="event-log-display">
            <div class="log-header">
              ...
              <span class="event-count">{{ eventLog.length || 0 }} {{ $t('vueIntegrationTests.events') }}</span>
            </div>
...
// Schema 示例（使用 Options 风格事件调用 $methods）
// 使用函数生成 schema，以便在语言切换时重新计算
const getSchemas = (): Record<string, Schema> => ({
  simpleForm: {
    type: 'div',
...
const { vnode, state, ctx } = useVario(currentSchema, {
  ...
  computed: {
    todoCompleted: (s) => (s.todos?.filter((t: any) => t.done).length) || 0,
    totalUsers: (s) => (s.users?.length) || 0
  },
  methods: {
    submitForm: ({ state }) => {
    ...
    decrementCounter: ({ state }) => { state.counter = (state.counter ?? 0) - 1 },
    incrementCounter: ({ state }) => { state.counter = (state.counter ?? 0) + 1 },
    addTodo: ({ state }) => {
    ...
    addUser: ({ state }) => {
      ...
      state.users.push({ id: nextId, name: `User ${nextId}`, email: `user${nextId}@example.com`, role: 'user' })
    }
  },
