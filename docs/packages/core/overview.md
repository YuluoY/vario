...
- **路径与循环**：`parsePath`、`getPathValue`、`setPathValue`、`createLoopContext` / `releaseLoopContext`- **Schema 工具**：`analyzeSchema`、`findNodes`、`createQueryEngine` 等框架无关的 Schema 分析与查询
...
| Schema 工具 | Schema 分析、查询、遍历（框架无关） | [Schema 工具](/packages/core/schema-utilities) |
